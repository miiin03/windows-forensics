...
EVTX(윈도우 이벤트 로그)를 파싱→정규화→SQLite 적재하고, **로컬 LLM(Qwen2.5) AI 조사관**이
자연어 질의를 받아 포렌식 도구를 스스로 호출해 근거 기반으로 답한다. 팀원 대시보드/코파일럿 UI와는
**로컬 HTTP 엔드포인트(`/ask`)**로 연동한다. 완전 오프라인·무료.
